...
class ODEFunc_quad(eqx.Module):
    ...
    def __call__(self, t, y):
        ...
        # this gives quadratic_weight applied to all (y_i,y_j)
        # repetitions have not been handled.
...
#         self.E = jr.normal(key1, (input_dim,)) * 0.01
#         self.L = jr.normal(key2, (input_dim, input_dim)) * 0.01
        
#         # Initialize Q with the energy-preserving constraint
#         Q_raw = jr.normal(key3, (input_dim, input_dim, input_dim)) * 0.01
#         Q_symmetric = (Q_raw + jnp.transpose(Q_raw, (1, 0, 2)) + jnp.transpose(Q_raw, (2, 1, 0))) / 3
#         self.Q = Q_symmetric - jnp.einsum('ijk->ikj', Q_symmetric) / 2

#     def __call__(self, t, y):
#         linear_term = jnp.dot(self.L, y)
#         quadratic_term = jnp.einsum('ijk,j,k->i', self.Q, y, y)
#         return self.E + linear_term + quadratic_term
